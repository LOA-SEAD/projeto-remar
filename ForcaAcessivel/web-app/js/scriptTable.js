   ...
   // addListeners();
    ...
    $('#save').click(function () {
        ...
        if(params.length) {
            ...
            window.top.location.href = "/forca_acessivel/question/toJson/" + params;
        }
        else{
            $('#totalQuestion').empty();
...
function save(tr) {
    ...
    var url = location.origin + '/forca_acessivel/question/save/';
...
function update(tr) {
    ...
    var url = location.origin + '/forca_acessivel/question/update/' + $(tr).attr('data-id');
    var data = { statement: $(tds)[1].textContent,
                 answer: $(tds)[2].textContent,
                 category: $(tds)[3].textContent,
                 author: $(tds)[4].textContent,
...
function _edit(tr){
    var url = location.origin + '/forca_acessivel/question/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            data: data,
            url: url,
            //url: "/forca_acessivel/question/edit",
    ...
    if(list_id.length<=0){
        ...
        if(list_id.length==1){
            if(confirm("Você tem certeza que deseja deletar essa questão?")){
                url = location.origin + '/forca_acessivel/question/delete/' + list_id[0];
                data = {_method: 'DELETE'};
                trID = "#tr"+list_id[0];
                $.ajax({
                        type: 'DELETE',
            ...
            if(confirm("Você tem certeza que deseja deletar essas questões?")){
                for(var i=0;i<list_id.length;i++){
                    url = location.origin + '/forca_acessivel/question/delete/' + list_id[i];
                    data = {_method: 'DELETE'};
                    trID = "#tr"+list_id[i];
                    $(trID).remove();
                    $.ajax({
...
function exportQuestions(){
    ...
    if(list_id.length<=0){
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/forca_acessivel/question/exportCSV",
