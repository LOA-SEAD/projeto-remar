...
<div id="list-phrase" class="content scaffold-list" role="main">
	...
	<table id="generalTable" class="highlight centered responsive-table">
		<thead>
		<tr>
		...
		<g:each in="${phraseInstanceList}" status="i" var="phraseInstance">
			<tr class="${(i % 2) == 0 ? 'even' : 'odd'}" data-id="${phraseInstance.id}">
				...
				<td><g:link action="show" id="${phraseInstance.id}">${fieldValue(bean: phraseInstance, field: "author")}</g:link></td>
			</tr>
		</g:each>
		</tbody>
	</table>
...
<script>
	...
	$('#save').click(function () {
		...
		if (params.length) {
			...
			alert("É necessário selecionar uma frase!!!")
		}
	});
</script>
</body>
