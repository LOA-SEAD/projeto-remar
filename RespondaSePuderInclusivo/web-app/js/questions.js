    ...
    if(randomQuestion>0){
        ...
        if(questions_level1 > randomQuestion && questions_level2 > randomQuestion && questions_level3 > randomQuestion){
            ...
            $.ajax({
                type: "POST",
                traditional: true,
                url: "/respondasepuderinclusivo/question/exportQuestions",
...
function _edit(tr){
    var url = location.origin + '/respondasepuderinclusivo/question/returnInstance/' + $(tr).attr('data-id');
    ...
    if(list_id.length<=0){
        ...
        if(list_id.length==1){
            if(confirm("Você tem certeza que deseja deletar essa questão?")){
                url = location.origin + '/respondasepuderinclusivo/question/delete/' + list_id[0];
                data = {_method: 'DELETE'};
                trID = "#tr"+list_id[0];
                $.ajax({
                        type: 'DELETE',
            ...
            for(var i=0;i<list_id.length;i++){
                    url = location.origin + '/respondasepuderinclusivo/question/delete/' + list_id[i];
                    data = {_method: 'DELETE'};
                    trID = "#tr"+list_id[i];
                    $(trID).remove();
                $.ajax({
...
function exportQuestions(){
    ...
    if(list_id.length<=0){
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/respondasepuderinclusivo/question/exportCSV",
