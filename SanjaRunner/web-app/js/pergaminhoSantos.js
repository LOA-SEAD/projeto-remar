...
function _modal_edit(tr){
    var url = location.origin + '/sanjarunner/pergaminhoSantos/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            success: function (returndata) {
                var pergaminhoSantosInstance = returndata.split("%@!");
                //pergaminhoSantosInstance é um vetor com os atributos da classe Information na seguinte ordem:
                ...
                $("#editInformation0").attr("value",pergaminhoSantosInstance[0]);
                $("#editInformation1").attr("value",pergaminhoSantosInstance[1]);
                $("#editInformation2").attr("value",pergaminhoSantosInstance[2]);
                $("#editInformation3").attr("value",pergaminhoSantosInstance[3]);
                $("#editInformation4").attr("value",pergaminhoSantosInstance[4]);
                $("#pergaminhoSantosID").attr("value",pergaminhoSantosInstance[5]);
                $("#editModal").openModal();
            },
...
/*function exportInformations(){
    ...
    $.each($("input[type=checkbox]:checked"), function(ignored, el) {
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/sanjarunner/pergaminhoSantos/exportCSV",
...
function _submit() {
    ...
    $.ajax({
        type: "POST",
        traditional: true,
        url: "/sanjarunner/pergaminhoSantos/exportInformations",
        data: { list_id: list_id},
        success: function(returndata) {
