...
function _modal_edit(tr){
    var url = location.origin + '/sanjarunner/quizMatriz/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            success: function (returndata) {
                var quizMatrizInstance = returndata.split("%@!");
                //quizMatrizInstance é um vetor com os atributos da classe Question na seguinte ordem:
                // Question - Answer[0] - Answer[1] - Answer[2] - Answer[3] - CorrectAnswer - ID

                switch(quizMatrizInstance[5]){
                    case '0':
                ...
                $("#editQuestion").attr("value",quizMatrizInstance[0]);
                ...
                $("#editAnswers0").attr("value",quizMatrizInstance[1]);
                $("#editAnswers1").attr("value",quizMatrizInstance[2]);
                $("#editAnswers2").attr("value",quizMatrizInstance[3]);
                $("#editAnswers3").attr("value",quizMatrizInstance[4]);
                $("#quizMatrizID").attr("value",quizMatrizInstance[6]);
                $("#editModal").openModal();
            },
...
function exportQuestions(){
    ...
    $.ajax({
        type: "POST",
        traditional: true,
        url: "/sanjarunner/quizMatriz/exportCSV",
...
function _submit() {
    ...
    $.ajax({
        type: "POST",
        traditional: true,
        url: "/sanjarunner/quizMatriz/exportQuestions",
        data: { list_id: list_id},
        success: function(returndata) {
