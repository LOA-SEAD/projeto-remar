...
function _modal_edit(tr){
    var url = location.origin + '/sanjarunner/quizVicentina/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            success: function (returndata) {
                var quizVicentinaInstance = returndata.split("%@!");
                //quizVicentinaInstance é um vetor com os atributos da classe Question na seguinte ordem:
                // Question - Answer[0] - Answer[1] - Answer[2] - Answer[3] - CorrectAnswer - ID

                switch(quizVicentinaInstance[5]){
                    case '0':
                ...
                $("#editQuestion").attr("value",quizVicentinaInstance[0]);
                ...
                $("#editAnswers0").attr("value",quizVicentinaInstance[1]);
                $("#editAnswers1").attr("value",quizVicentinaInstance[2]);
                $("#editAnswers2").attr("value",quizVicentinaInstance[3]);
                $("#editAnswers3").attr("value",quizVicentinaInstance[4]);
                $("#quizVicentinaID").attr("value",quizVicentinaInstance[6]);
                $("#editModal").openModal();
            },
...
function exportQuestions(){
    ...
    $.ajax({
        type: "POST",
        traditional: true,
        url: "/sanjarunner/quizVicentina/exportCSV",
...
function _submit() {
    ...
    $.ajax({
        type: "POST",
        traditional: true,
        url: "/sanjarunner/quizVicentina/exportQuestions",
        data: { list_id: list_id},
        success: function(returndata) {
