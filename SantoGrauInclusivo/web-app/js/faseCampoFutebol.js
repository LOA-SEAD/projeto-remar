...
function _modal_edit(tr){
    var url = location.origin + '/santograuinclusivo/faseCampoFutebol/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            success: function (returndata) {
                var faseCampoFutebolInstance = returndata.split("%@!");
                // faseCampoFutebolInstance é um vetor com os atributos da classe Question na seguinte ordem:
                // faseCampoFutebolInstance[] = [Title, Answer, ID]

                $("#labelTitle").attr("class","active");
                $("#labelAnswer").attr("class","active");
                $("#editTitle").attr("value",faseCampoFutebolInstance[0]);
                $("#editAnswer").attr("value",faseCampoFutebolInstance[1]);
                $("#faseCampoFutebolID").attr("value",faseCampoFutebolInstance[2]);
                $("#editModal").openModal();
            },
...
function _delete() {
    ...
    if(list_id_delete.length==1){
        url = location.origin + '/santograuinclusivo/faseCampoFutebol/delete/' + list_id_delete[0];
        data = {_method: 'DELETE'};
        ...
        for(var i=0;i<list_id_delete.length;i++){
            url = location.origin + '/santograuinclusivo/faseCampoFutebol/delete/' + list_id_delete[i];
            data = {_method: 'DELETE'};
...
function exportQuestions(){
    ...
    if(list_id.length<=0){
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/santograuinclusivo/faseCampoFutebol/exportCSV",
...
function _submit() {
    ...
    if($("input[type=checkbox]:checked").size() < 2) {
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/santograuinclusivo/faseCampoFutebol/exportQuestions",
            data: { list_id: list_id},
            success: function(returndata) {
