...
function _modal_edit(tr){
    var url = location.origin + '/santograuinclusivo/faseCampoMinado/returnInstance/' + $(tr).attr('data-id');
...
function _delete() {
    ...
    if(list_id_delete.length==1){
        url = location.origin + '/santograuinclusivo/faseCampoMinado/delete/' + list_id_delete[0];
        data = {_method: 'DELETE'};
        ...
        for(var i=0;i<list_id_delete.length;i++){
            url = location.origin + '/santograuinclusivo/faseCampoMinado/delete/' + list_id_delete[i];
            data = {_method: 'DELETE'};
...
function exportQuestions(){
    ...
    if(list_id.length<=0){
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/santograuinclusivo/faseCampoMinado/exportCSV",
...
function _submit() {
    ...
    if($("input[type=checkbox]:checked").size() < 4) {
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/santograuinclusivo/faseCampoMinado/exportQuestions",
            data: { list_id: list_id},
            success: function(returndata) {
